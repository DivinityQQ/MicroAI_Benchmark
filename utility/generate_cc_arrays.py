...
def generate_file(out_fname, array_name, array_type, array_contents, size):
  ...
  if out_fname.endswith('.cc'):
    ...
    out_cc_file.write('alignas(16) const {} {}[] = {{\n'.format(
...
def bytes_to_hexstring(buffer):
  """Convert a byte array to a hex string with formatting."""
  hex_values = []
  for i in range(len(buffer)):
    hex_string = "0x{:02x}".format(buffer[i])  # Format to 2 hex digits with leading zero
    hex_values.append(hex_string)

  # Format into rows of 12
  formatted_string = ""
  for i in range(0, len(hex_values), 12):
    row = hex_values[i:i+12]
    formatted_string += "  " + ", ".join(row) + ",\n" # Add indentation and comma at the end of each row
